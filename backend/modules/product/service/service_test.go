...
func (s *ProductServiceTestSuite) TestGetTotalProducts_Success() {
	...
	s.repository.On("GetTotalProducts", mock.Anything, query).Return(expectedTotal, nil)

	total, err := s.service.productRepository.GetTotalProducts(context.Background(), query)
